...
namespace Separating
{
    public abstract class BaseIni
    {
        ...
        public bool ReadBool(string section, string ident, bool Default)
        {
            try
            {
                return Convert.ToBoolean(Convert.ToInt32((ReadString(section, ident, Convert.ToString(Default))))); //we want same behavior as Delphi, ie 1=true, 0=false
            }
            catch { return Default; }
        }
        ...
        public void WriteBool(string section, string ident, bool value)
        {
            try
            {
                WriteString(section, ident, Convert.ToString(Convert.ToInt32(value)));
            }
            catch (Exception ex) { throw new Exception(ex.Message); }
        }
